...
class UserSequenceDataset(Dataset):
    """Dataset to load user transaction sequences from a CSV file."""

    def __init__(self, csv_path, tokenizer, context_length):
    ...
    def __getitem__(self, idx):
        sequence_str = self.df.iloc[idx]["transaction_history"]
        token_ids = self.tokenizer.tokenizer.encode(sequence_str).ids
        full_sequence = torch.tensor(token_ids, dtype=torch.long)
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        for i, (x, y) in enumerate(dataloader):
            ...
            if i % 50 == 0:
                print(
                    f"Epoch {epoch+1}/{config.epochs}, Batch {i}/{len(dataloader)}, Loss: {loss.item():.4f}"
                )
